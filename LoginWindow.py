...
class LoginDialog(QLoginDialog, Ui_Dialog):  # 定义一个名为LoginDialog的类，它继承自FLoginDialog和Ui_Dialog
    ...
    def setUiStyle(self, windowFlag=False, transBackFlag=False):  # 定义setUiStyle函数，用于设置用户界面的样式
        """
        Function: Setting the state of UI controls, author: Fredy,
        website: https://www.bilibili.com/video/BV16K411Q7YX/;
        https://www.bilibili.com/video/BV16K411Q7YX/
        :param windowFlag:
        :param transBackFlag:
        :return:
        """
        ...
        # 设置各个输入框的占位符文本
        self.lineEdit_user_log.setPlaceholderText("User Name")
        self.lineEdit_password.setPlaceholderText("Password")
        self.lineEdit_user_reg.setPlaceholderText("User Name")
        self.lineEdit_password_reg.setPlaceholderText("Password")
        self.lineEdit_code_reg.setPlaceholderText("Verification Code")
    ...
    def do_forget(self):  # 定义do_forget函数，用于处理用户点击忘记密码后的操作
        ...
        self.pushButton_reg.setText("Modify Password")  # 将注册按钮的文本改为"修改密码"
    ...
    def do_avatar(self):  # 定义do_avatar函数，用于处理用户点击加载头像按钮后的操作
        ...
        if reply == -1:  # 如果文件不存在，显示提示信息
            self.label_reg_info.setText("File does not exist.")
        elif reply == -2:  # 如果读取头像失败，显示提示信息
            self.label_reg_info.setText("Failed to read avatar")
        elif reply == 0:  # 如果头像文件有效，显示提示信息，并将头像文件的路径保存到self.avatar中
            self.label_reg_info.setText("Valid avatar file")
    ...
    def do_login(self):  # 定义do_login函数，用于处理用户点击登录按钮后的操作
        ...
        if name_edit != "" and pwd_edit != "":  # 如果用户名和密码都不为空
            if self.pushButton_login.text() == "Login":  # 如果登录按钮的文本是"登 录"
                # 尝试验证用户名和密码
                reply = self.user_manager.verify_login(name_edit, pwd_edit)
                if reply == -2:  # 如果密码不正确，显示提示信息
                    self.label_log_info.setText("Incorrect Password")
                elif reply == -1:  # 如果用户未注册，显示提示信息
                    self.label_log_info.setText("User not registered.")
                    ...
                    self.label_log_info.setText("Logging in...")  # 显示正在登录的提示信息
            ...
            elif self.pushButton_login.text() == "Account Cancellation":  # 如果登录按钮的文本是"注销账户"
                reply = self.user_manager.delete_user(name_edit, pwd_edit)  # 尝试注销用户
                if reply == -2:  # 如果密码不正确，显示提示信息
                    self.label_log_info.setText("Incorrect Password")
                elif reply == -1:  # 如果用户名不存在，显示提示信息
                    self.label_log_info.setText("User name does not exist !")
                elif reply == 0:  # 如果用户名和密码都正确，用户成功注销
                    self.label_log_info.setText("Account deleted successfully !")
                    QtWidgets.QApplication.processEvents()  # 处理所有事件，以更新界面
                    time.sleep(3)  # 等待3秒
                    self.close()  # 关闭登录对话框
        else:  # 如果用户名或密码为空，显示提示信息
            self.label_log_info.setText("Incomplete Information")
            ...
            if ver_edit.lower() == self.ver_code.lower():  # 如果输入的验证码（忽略大小写）与生成的验证码一致
                if self.pushButton_reg.text() == "Register":  # 如果注册按钮的文本是"注 册"
                    ...
                    if reply == 0:  # 如果注册成功，显示提示信息
                        self.label_reg_info.setText("Registration Succeeded")
                    elif reply == -1:  # 如果用户已被注册过，显示提示信息
                        self.label_reg_info.setText("User has been registered")
                    elif reply == -2:  # 如果密码长度过短，显示提示信息
                        self.label_reg_info.setText("Password length is too short")
                    elif reply == -3:  # 如果没有选择头像文件，显示提示信息
                        self.label_reg_info.setText("Select an avatar file")

                elif self.pushButton_reg.text() == "Modify Password":  # 如果注册按钮的文本是"修改密码"
                    reply = self.user_manager.change_password(name_edit, pwd_edit)  # 尝试修改密码
                    if reply == 0:  # 如果修改密码成功，显示提示信息
                        self.label_reg_info.setText("Password modification succeeded")
                        ...
                        self.label_reg_info.setText("User name does not exist")
                    elif reply == -2:  # 如果密码长度过短，显示提示信息
                        self.label_reg_info.setText("Password length is too short")

                elif self.pushButton_reg.text() == "Modify Avatar":  # 如果注册按钮的文本是"修改头像"
                    # 获取
                    reply = self.user_manager.change_avatar(name_edit, pwd_edit, self.avatar)  # 尝试修改头像
                    if reply == 0:  # 如果修改头像成功，显示提示信息
                        self.label_reg_info.setText("Successfully modified avatar !")
                        ...
                        self.label_reg_info.setText("Password is incorrect")
                    elif reply == -3:  # 如果没有选择头像文件，显示提示信息
                        self.label_reg_info.setText("Select an avatar file")
            else:  # 如果验证码错误，显示提示信息
                self.label_reg_info.setText("Verification code error !")
        else:  # 如果用户名、密码或验证码为空，显示提示信息
            self.label_reg_info.setText("Incomplete Information")
